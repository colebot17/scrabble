        ...
        data = function() {
            function d(id) {
                ...
                this.setupCanvasContext = function() {
                    ...
                    d.CTX = canvas.getContext("2d"), canvas.width = 2 * window.innerWidth, canvas.height = 2 * window.innerHeight, canvas.style.position = "fixed", canvas.style.top = "0", canvas.style.left = "0", canvas.style.width = "calc(100%)", canvas.style.height = "calc(100%)", canvas.style.margin = "0", canvas.style.padding = "0", canvas.style.zIndex = "999999999", canvas.style.pointerEvents = "none", document.body.appendChild(canvas), window.addEventListener("resize", function() {
                        canvas.width = 2 * window.innerWidth, canvas.height = 2 * window.innerHeight
                    });
                };
                this.setupElement = function(elId) {
                    ...
                    el.addEventListener("click", function(event) {
                        // add a burst at the mouse position
                        var clickPt = new OrderedPair(2 * event.clientX, 2 * event.clientY);
                        animation.bursts.push(new Burst(clickPt));
                        // hide the clicked element if destroyTarget set
                        if (d.CONFIG.destroyTarget) animation.element.style.visibility = "hidden";
                    });
                };
