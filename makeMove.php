...
if ((int)$players[$turn]['id'] === (int)$user && (int)$inactive === 0) { // make sure it is actually the user's turn and that the game is active
	...
	if (count($players[array_search($user, $playerList)]['letterBank']) === 0 && count($longBag) === 0) {
		// end the game
		$inactive = 1;
	}
	...
	if (!$inactive && count($longBag) > 0) {
		...
		while (count($players[array_search($user, $playerList)]['letterBank']) < 7) {
			...
			$letterBag[$newLetter]--;
			array_push($players[array_search($user, $playerList)]['letterBank'], $newLetter);
		}
	}
	...
	unset($players[array_search($user, $playerList)]['subsequentSkips']);
	...
	echo '{"errorLevel":1,"message":"It isn\'t your turn!"}';
}
