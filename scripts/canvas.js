	...
	for (var y = 0; y < squareNum; y++) { // for each tile
		for (var x = 0; x < squareNum; x++) {
			...
			if ( boardMultiplierStrings[boardModifiers[y][x]]) {
				...
				canvas.ctx.fillText(boardMultiplierStrings[boardModifiers[y][x]], (x * squareWidth) + (x * squareGap) + (squareWidth / 2), (y * squareWidth) + (y * squareGap) + (squareWidth / 2) + (fontSize / 2.3));
			}
		}
	}
}
