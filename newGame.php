...
if (password_verify($userPwd, $row['pwd'])) {
	...
	// generate a lightweight player list to go in the user's game list
	$numPlayers = count($playerList);
	$playerListNames = array(); // create an array to store names in
	for ($i = 0; $i < $numPlayers; $i++) { // for each player in the game
		$sql = "SELECT name FROM accounts WHERE id='$playerList[$i]'";
		$query = mysqli_query($conn, $sql);
		$row = mysqli_fetch_assoc($query);
	...
	// add the game
	$sql = "INSERT INTO games(letterBag, players, turn, inactive, board, lastMove, chat) VALUES ('$letterBagJson', '$playersJson', 0, 0, '$boardJson', '$timestamp', '[]');";
	$query = mysqli_query($conn, $sql);
